...
/*   Updated: 2025/03/18 17:58:07 by lsilva-x         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_philo(t_data *philo_s)
{
	...
	if (philo_s == NULL)
		return ;
	sem_destroy(philo_s->s_write);
	if (philo_s->s_forks != NULL)
	{
		i = -1;
		while (++i < philo_s->philo_num)
			sem_destroy(&philo_s->s_forks[i]);
		free(philo_s->s_forks);
		philo_s->s_forks = NULL;
	}
	free_node(philo_s);
}
